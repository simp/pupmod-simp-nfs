  ...
  context 'with default parameters' do
    ...
    it do
      is_expected.to create_exec('enable_nfsidmap_request_key').with(
        ...
        command: "/usr/bin/sed -r -i '/^create[[:space:]]+id_resolver[[:space:]]/d' /etc/request-key.conf;" \
          "/usr/bin/sed -i '/^negate/i create\tid_resolver\t*\t*\t\t/usr/sbin/nfsidmap -t 600 %k %d' /etc/request-key.conf",
      )
    end
  end
end
